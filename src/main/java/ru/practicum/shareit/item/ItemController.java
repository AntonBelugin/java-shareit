...
public class ItemController {
    ...
    public ItemDtoResponse create(@RequestHeader(HEADER_USER) long userId, @Valid @RequestBody ItemDtoRequest item) {
        return itemService.add(userId, item);
    }

    @GetMapping("/{id}")
    public ItemDtoWithComments findById(@RequestHeader(HEADER_USER) long userId, @PathVariable long id) {
        return itemService.findById(userId, id);
    }

    @PatchMapping("/{id}")
    public ItemDtoResponse update(@RequestHeader(HEADER_USER) long userId, @PathVariable long id,
                                  @RequestBody ItemDtoRequest item) {
        return itemService.update(userId, id, item);
    }

    @GetMapping
    public List<ItemDtoWithComments> findItemsByUser(@RequestHeader(HEADER_USER) long userId) {
    ...
    public CommentDtoResponse create(@RequestHeader(HEADER_USER) long userId,
